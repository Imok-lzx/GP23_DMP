...
object TagContext2 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    if(!hbadmin.tableExists(TableName.valueOf(HbaseTableName))){
      ...
      // 创建列簇
      val hColumnDescriptor = new HColumnDescriptor("tags2")
      // 将创建好的列簇加入表中
    ...
    val verties = allUserId.flatMap(row=>{
      ...
      val adList = TasAd.makeTags(rows)
      // 商圈
      val businessList = BusinessTag.makeTags(rows)
      // 媒体标签
      val appList = AppnameTag.makeTags(rows,broadValue)
      // 设备标签
      val devList = EquipmentTag.makeTags(rows)
      // 地域标签
      val locList = TagsLocation.makeTags(rows)
      // 关键字标签
      val kwList = KeywordTag.makeTags(rows,broadValues)
      // 获取所有的标签
    ...
    vertices.join(verties).map{
      ...
      case (userId,userTags) =>{
        ...
        put.addImmutable(Bytes.toBytes("tags2"),Bytes.toBytes(day),Bytes.toBytes(userTags.mkString(",")))
