...
//     调用HbaseAPI
    ...
    df.map(row=>{
      ...
      // 接下来标签 实现
      val adList = TasAd.makeTags(row)
      // 商圈
      val businessList = BusinessTag.makeTags(row)
      // 媒体标签
      val appList = AppnameTag.makeTags(row,broadValue)
      // 设备标签
      val devList = EquipmentTag.makeTags(row)
      // 地域标签
      val locList = TagsLocation.makeTags(row)
      // 关键字标签
      val kwList = KeywordTag.makeTags(row,broadValues)
